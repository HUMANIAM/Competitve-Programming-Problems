...
/*                                              Matrix incrementation
                                                ----------------------
Description: we have N*N matrix with zero elements and you given q queries (a,b), (c, d) to increment elements in this range
-----------  then print the matrix after apply multiplication
*/

void MatInc(){
    int q, n;
cin>>n>>q;
	    int ar[n][n],A[n][n],B[n][n];
...
//                      **********************
//idea is sorting a structure according to specific value in structure
struct Order{
    int o, p, d;
};
int N, X, Y;
int orders[110][2];
vector<struct Order> ords(110);

bool comparator(int i, int j){
    return ords[i].d > ords[j].d ? true : false;
}
...
int maxProfit(void){
    ...
    for(int i=0; i<N; i++){
        pos[i] = i;
        ords[i].o = i;
        ords[i].d = abs(orders[i][0]-orders[i][1]);
        if(orders[i][0]>orders[i][1])
            ords[i].p=0;
        else ords[i].p=1;
    }
    sort(pos, pos+N, comparator);

    int maxsum = 0;
    for(int i=0; i<N; i++){
        if(ords[pos[i]].p == 0 && X>0){
            X--;
            maxsum += orders[ords[pos[i]].o][0];
        }else if(X==0 || Y>0){
            Y--;
            maxsum += orders[ords[pos[i]].o][1];
        }else{
            X--;
            maxsum += orders[ords[pos[i]].o][0];
        }
...
///O(nklogk)
void findSubMax(int* a, int n, int k){
    int subarr[k];
    for(int i=0; i<k; i++) subarr[i] = a[i];
